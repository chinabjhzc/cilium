...
func (p *DNSProxy) checkRestored(endpointID uint64, destPort uint16, destIP string, name string) bool {
	ruleIPs, exists := p.restored[endpointID][destPort]
	if !exists {
		return false
	}

	for re, ips := range ruleIPs {
		if _, exists := ips[destIP]; exists && re.MatchString(name) {
			return true
		}
	}
	return false
...
func (p *DNSProxy) GetRules(endpointID uint16) restore.DNSRules {
	...
	for port, entries := range p.allowed[uint64(endpointID)] {
		...
		for cs, regex := range entries {
			...
			ruleIPs[restore.RuleRegex{Regexp: regex}] = IPs
		}
		restored[port] = ruleIPs
	}
	return restored
}
